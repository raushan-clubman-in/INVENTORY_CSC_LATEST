...
#End Region 
    ...
    Private Sub fgrdCompanyList_Click1()
        ...
        If Trim(fgrdCompanyList.get_TextMatrix(fgrdCompanyList.Row, 0)) <> "" Then
            ...
            'If MsgBox("ReProcess Negative Stock", MsgBoxStyle.OKCancel + MsgBoxStyle.DefaultButton1, "Stock Alert") = MsgBoxResult.OK Then
            '    Me.Cursor = Cursors.WaitCursor
            '    gconnection.openConnection()
            '    gcommand = New SqlCommand("CP_STOCKSUMMARY_MINUS", gconnection.Myconn)
            '    gcommand.CommandType = CommandType.StoredProcedure
            '    gcommand.Parameters.Add(New SqlParameter("@FROMDATE", SqlDbType.DateTime)).Value = Format(CDate("01/04/" & gFinancalyearStart), "dd/MM/yyyy")
            '    gcommand.Parameters.Add(New SqlParameter("@TODATE", SqlDbType.DateTime)).Value = Format(CDate(Now), "dd/MM/yyyy")
            ...
            'SQLSTRING = SQLSTRING & "ISNULL(S.CLSRATE,0) AS RATE, ISNULL(S.OPQTY,0.000) AS OPQTY, ISNULL(S.RCVQTY,0.000) AS RCVQTY, ISNULL(S.ISSQTY,0.000) AS ISSQTY, ISNULL(S.CLSQTY,0.00) AS CLSQTY, ISNULL(S.CLSVAL,0.00) AS VALUE,ISNULL(S.STORECODE,'') AS STORECODE FROM STOCKSUMMARY_MINUS S, INVENTORYITEMMASTER I "
            'SQLSTRING = SQLSTRING & " WHERE I.ITEMCODE=S.ITEMCODE and I.storecode=s.storecode "
            'SQLSTRING = SQLSTRING & " And CLSQTY < 0"
            'SQLSTRING = SQLSTRING & " ORDER BY I.GROUPNAME,S.ITEMNAME,S.STORECODE   "
            'Dim heading() As String = {"NEGATIVE STOCK SUMMARY"}
            'Dim ObjSTOCKSummary As New rptStockSummaryminus
            'ObjSTOCKSummary.Reportdetails(SQLSTRING, heading, Format(CDate("01/04/" & gFinancalyearStart), "dd/MM/yyyy"), Format(CDate(Now), "dd/MM/yyyy"))
    ...
    Private Sub CompanyList1_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        SQLSTRING = SQLSTRING & " ISNULL(ADD1,'') AS ADD1,ISNULL(ADD2,'') AS ADD2,ISNULL(CITY,'') AS CITY,ISNULL(STATE,'') AS STATE,ISNULL(PINCODE,'') AS PINCODE,ISNULL(PHONE1,'') AS PHONE1,ISNULL(FAX,'') AS FAX FROM CLUBMASTER WHERE ISNULL(ACTIVE,'')='Y' ORDER BY SNO"
