...
Public Class RptItemmasterstk
    ...
    Public Function Reportdetails(ByVal SQLSTRING As String, ByVal PAGEHEAD() As String, ByVal FROMDATE As Date, ByVal TODATE As Date)
        ...
        Try
            ...
            If gdataset.Tables("ISSUESUMMARYREPORT").Rows.Count > 0 Then
                ...
                'Filewrite.WriteLine()
                'Filewrite.WriteLine(Chr(15) & Trim(gdataset.Tables("ISSUESUMMARYREPORT").Rows(0).Item("opstorelocationcode")) & "->" & GROUPNAME)
                'Filewrite.WriteLine()
                'pagesize = pagesize + 3
                For Each dr In gdataset.Tables("ISSUESUMMARYREPORT").Rows
                    ...
                    '    Filewrite.WriteLine("{0,32}{1,12}{2,24}{3,12}", " TOTAL =======>        ", Format(GQTY, "0.000"), "", Format(GVALUE, "0.00"))
                    ...
                    If ITEMCODE <> Trim(CStr(dr("ITEMCODE"))) Then
                        ...
                        Filewrite.Write("{0,-12}", Mid(Trim(CStr(dr("UOM"))), 1, 23))
                        Filewrite.Write("{0,12}", Mid(Format(Val(dr("CLSQTY")), "0.000"), 1, 12))
                        TGROUPQty = TGROUPQty + Val(dr("CLSQTY"))
                        Filewrite.Write("{0,12}", Mid(Format(Val(dr("CLSRATE")), "0.00"), 1, 12))
                        Filewrite.WriteLine("{0,12}", Mid(Format(Val(dr("CLSQTY")) * Val(dr("CLSRATE")), "0.00"), 1, 12))
                        Tgroupvalue = Tgroupvalue + (Val(dr("CLSQTY")) * Val(dr("CLSRATE")))
                        pagesize = pagesize + 1
                    End If
                Next dr
                Filewrite.WriteLine()
                ...
                Filewrite.WriteLine("{0,44}{1,12}{2,12}{3,12}", "GRAND TOTAL =======>        ", Format(TGROUPQty, "0.000"), "", Format(Tgroupvalue, "0.00"))
                pagesize = pagesize + 1
                Filewrite.WriteLine(StrDup(80, "="))
                pagesize = pagesize + 1
            Else
    ...
    Private Function PrintHeader(ByVal Heading() As String, ByVal mskfromdate As Date, ByVal msktodate As Date)
        ...
        Try
            ...
            pagesize = pagesize + 1
            Filewrite.WriteLine("{0,-30}{1,-26}{2,-30}{3,-25}{4,-24}", Mid(Address1, 1, 30), " ", Mid(Trim(Heading(0)), 1, 35), " ", "01-04-" & gFinancalyearStart & " TO 31-03-" & gFinancialyearEnd)
            pagesize = pagesize + 1
            Filewrite.WriteLine("{0,-30}{1,-26}{2,-30}", Mid(Address2, 1, 30), " ", Mid(StrDup(Len(Trim(Heading(0))), "-"), 1, 35))
            pagesize = pagesize + 1
            ...
            Filewrite.WriteLine(StrDup(80, "-"))
            pagesize = pagesize + 1
            Filewrite.WriteLine("{0,-8}{1,-24}{2,-12}{3,12}{4,12}{5,12}", "ITEM", "ITEM", "UOM", "CLSQTY", "CLSRATE", "CLSVALUE")
            pagesize = pagesize + 1
            Filewrite.WriteLine("{0,-8}{1,-24}{2,-12}{3,12}{4,12}{5,12}", "CODE", "DESCRIPTION", "", "", "", "")
            pagesize = pagesize + 1
            Filewrite.WriteLine(StrDup(80, "-"))
            pagesize = pagesize + 1
        Catch ex As Exception
