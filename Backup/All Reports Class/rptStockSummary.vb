...
Public Class rptStockSummary
    ...
    Public Function Reportdetails_withoutGroup(ByVal SQLSTRING As String, ByVal PAGEHEAD() As String, ByVal FROMDATE As Date, ByVal TODATE As Date)
        Dim dbldocTotal, dblGroupTotal, dblGrandTotal, CVALUE, OFN As Double
        Dim ITEMCODE, DOCDETAILS, GROUPNAME, SUBGROUPNAME As String
        Dim ITEMBOOL, DOCBOOL As Boolean
        Dim opval, issval, closeval, rcval, groupvalue As Double
        Dim Topval, Tissval, Tcloseval, Trcval, Tgroupvalue As Double
        Dim I, OQB, OQM As Integer
        Try
            Call Randomize()
            AppPath = Application.StartupPath
            vOutfile = Mid("Ste" & (Rnd() * 800000), 1, 8)
            ...
            If gdataset.Tables("STOCKSUMMARYREPORT").Rows.Count > 0 Then
                ...
                For Each dr In gdataset.Tables("STOCKSUMMARYREPORT").Rows
                    ...
                    If GROUPNAME <> Trim(CStr(dr("GROUPNAME"))) Then
                        GROUPNAME = Trim(CStr(dr("GROUPNAME")))
                        Filewrite.WriteLine(StrDup(118, "."))
                        If UCase(Mid(MyCompanyName, 1, 5)) = "CUTTA" Then
                            Filewrite.WriteLine("{0,54}{1,12}{2,12}{3,12}{4,12}{5,12}", " TOTAL =======>        ", Format(opval, "0.00"), Format(rcval, "0.00"), Format(issval, "0.00"), Format(closeval, "0.00"), Format(groupvalue, "0.00"))
                        Else
                            Filewrite.WriteLine("{0,54}{1,12}{2,12}{3,12}{4,12}{5,12}", " TOTAL =======>        ", Format(opval, "0.000"), Format(rcval, "0.000"), Format(issval, "0.000"), Format(closeval, "0.000"), Format(groupvalue, "0.00"))
                        End If
                        Topval = Topval + opval
                    ...
                    If SUBGROUPNAME <> Trim(CStr(dr("SUBGROUPNAME"))) Then
                        ...
                        Filewrite.WriteLine()
                        Filewrite.WriteLine(Chr(15) & SUBGROUPNAME)
                        Filewrite.WriteLine(StrDup(Len(SUBGROUPNAME), "."))
                        pagesize = pagesize + 3
                    End If
                    zeroval = 0
                    Filewrite.Write("{0,-8}", Chr(15) & Mid(Trim(CStr(dr("ITEMCODE"))), 1, 8))
                    ...
                    Else
                        Filewrite.Write("{0,12}", Mid(Format(zeroval, "0.00"), 1, 12))
                    End If
                    If UCase(Mid(MyCompanyName, 1, 5)) = "INTER" And (Val(dr("CONVALUE")) = 750 Or Val(dr("CONVALUE")) = 1000) Then
                        CVALUE = Val(dr("CONVALUE"))
                        OQB = Floor((dr("OPQTY") * CVALUE) / CVALUE)
                        OQM = (dr("OPQTY") * CVALUE) Mod CVALUE
                        OFN = OQM / 1000
                        OFN = OQB + OFN
                        Filewrite.Write("{0,12}", Mid(Format(Val(OFN), "0.000"), 1, 12))
                        opval = opval + Val(OFN)
                        OQB = Floor((dr("RCVQTY") * CVALUE) / CVALUE)
                        OQM = (dr("RCVQTY") * CVALUE) Mod CVALUE
                        OFN = OQM / 1000
                        OFN = OQB + OFN
                        Filewrite.Write("{0,12}", Mid(Format(Val(OFN), "0.000"), 1, 12))
                        rcval = rcval + Val(OFN)
                        OQB = Floor((dr("ISSQTY") * CVALUE) / CVALUE)
                        OQM = (dr("ISSQTY") * CVALUE) Mod CVALUE
                        OFN = OQM / 1000
                        OFN = OQB + OFN
                        Filewrite.Write("{0,12}", Mid(Format(Val(OFN), "0.000"), 1, 12))
                        issval = issval + Val(OFN)
                        OQB = Floor((dr("CLSQTY") * CVALUE) / CVALUE)
                        OQM = (dr("CLSQTY") * CVALUE) Mod CVALUE
                        OFN = OQM / 1000
                        OFN = OQB + OFN
                        Filewrite.Write("{0,12}", Mid(Format(Val(OFN), "0.000"), 1, 12))
                        closeval = closeval + Val(OFN)
                        Filewrite.WriteLine("{0,12}", Mid(Format(Val(dr("VALUE")), "0.00"), 1, 12))
                        groupvalue = groupvalue + Val(dr("VALUE"))
                        dblGrandTotal = dblGrandTotal + Format(Val(dr("VALUE")), "0.00")

                    ElseIf UCase(Mid(MyCompanyName, 1, 5)) = "CUTTA" And (Val(dr("CONVALUE")) = 750 Or Val(dr("CONVALUE")) = 1000) Then
                        CVALUE = Val(dr("CONVALUE"))
                        OQB = Floor((dr("OPQTY") * CVALUE) / CVALUE)
                        OQM = (dr("OPQTY") * CVALUE) Mod CVALUE
                        OFN = OQM / 25
                        OFN = (Math.Round(OFN) / 100)
                        OFN = OQB + OFN
                        Filewrite.Write("{0,12}", Mid(Format(Val(OFN), "0.00"), 1, 12))
                        opval = opval + Val(OFN)
                        OQB = Floor((dr("RCVQTY") * CVALUE) / CVALUE)
                        OQM = (dr("RCVQTY") * CVALUE) Mod CVALUE
                        OFN = OQM / 25
                        ...
                        closeval = closeval + Val(OFN)
                        If Val(OFN) = 0 Then
                            Filewrite.WriteLine("{0,12}", Mid(Format(0, "0.00"), 1, 12))
                            groupvalue = groupvalue + 0
                            dblGrandTotal = dblGrandTotal + 0
                        Else
                            Filewrite.WriteLine("{0,12}", Mid(Format(Val(dr("VALUE")), "0.00"), 1, 12))
                            groupvalue = groupvalue + Val(dr("VALUE"))
                            dblGrandTotal = dblGrandTotal + Format(Val(dr("VALUE")), "0.00")
                ...
                pagesize = pagesize + 1
                If UCase(Mid(MyCompanyName, 1, 5)) = "CUTTA" Then
                    Filewrite.WriteLine("{0,54}{1,12}{2,12}{3,12}{4,12}{5,12}", "GRAND TOTAL =======>        ", Format(Topval, "0.00"), Format(Trcval, "0.00"), Format(Tissval, "0.00"), Format(Tcloseval, "0.00"), Format(Tgroupvalue, "0.00"))
                Else
                    Filewrite.WriteLine("{0,54}{1,12}{2,12}{3,12}{4,12}{5,12}", "GRAND TOTAL =======>        ", Format(Topval, "0.000"), Format(Trcval, "0.000"), Format(Tissval, "0.000"), Format(Tcloseval, "0.000"), Format(Tgroupvalue, "0.00"))
                End If
                pagesize = pagesize + 2
