...
Imports System.io
Public Class rptAbcIssueregister
            ...
            If gdataset.Tables("ISSUEREGISTERREPORT").Rows.Count > 0 Then
                ...
                For Each dr In gdataset.Tables("ISSUEREGISTERREPORT").Rows
                    If pagesize > 56 Then
                        ...
                        pagesize = pagesize + 1
                    End If
                    If Trim(STROEDESC) <> Trim(CStr(dr("ABC"))) Then
                        If STOREBOOL = True Then
                            Filewrite.WriteLine()
                            pagesize = pagesize + 1
                            Filewrite.WriteLine(StrDup(135, "*"))
                            pagesize = pagesize + 1
                            Filewrite.WriteLine("{0,-50}{1,-15}{2,-10}{3,12}{4,10}", "", "GROUP TOTAL =====>", "", "", Format(Val(dblGroupTotal), "0.00"))
                            pagesize = pagesize + 1
                            Filewrite.WriteLine(StrDup(135, "*"))
                            pagesize = pagesize + 1
                            Filewrite.WriteLine("{0,-15}", "CATEGORY:" & Mid(Trim(CStr(dr("ABC"))), 1, 15))
                            pagesize = pagesize + 1
                            STROEDESC = dr("ABC")
                ...
                Filewrite.WriteLine(StrDup(135, "*"))
                pagesize = pagesize + 1
                Filewrite.WriteLine("{0,-50}{1,-15}{2,-10}{3,12}{4,5}", "", "GROUP TOTAL =====>", "", "", Format(Val(dblGroupTotal), "0.00"))
                pagesize = pagesize + 1
                Filewrite.WriteLine(StrDup(135, "*"))
                pagesize = pagesize + 1
                Filewrite.WriteLine(StrDup(135, "="))
                pagesize = pagesize + 1
                Filewrite.WriteLine("{0,-50}{1,-15}{2,-10}{3,12}{4,5}", "", "GRAND TOTAL =====>", "", "", Format(Val(dblGrandTotal), "0.00"))
                pagesize = pagesize + 1
                Filewrite.WriteLine(StrDup(135, "="))
                pagesize = pagesize + 1
            Else
    ...
    Private Function PrintHeader(ByVal Heading() As String, ByVal mskfromdate As Date, ByVal msktodate As Date)
        ...
        Try
            ...
            Filewrite.WriteLine(StrDup(135, "-"))
            pagesize = pagesize + 1
            Filewrite.Write("{0,-20}{1,-15}{2,-15}{3,-10}{4,-5}", "ITEMCODE", "ITEMNAME", "", "UOM", "QUANTITY")
            Filewrite.WriteLine("{0,-5}{1,10}{2,15}", "", "RATE", "AMOUNT")
            pagesize = pagesize + 1
            Filewrite.Write("{0,-20}{1,-15}{2,-15}{3,-35}{4,-10}", "", "", "", "", "")
