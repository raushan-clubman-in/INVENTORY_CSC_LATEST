...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Chk_SelectPayment.Size = New System.Drawing.Size(132, 31)
        ...
        Me.Label10.Size = New System.Drawing.Size(404, 31)
...
#End Region
    ...
    Private Sub GridOperation()
        Try
            ...
            If Mid(Trim(gCompanyname), 1, 5) = "CALCU" Then
                strsql = "EXEC PJV_POSTING " & "'" & Format(Dtp_FromVoucherDate.Value, "dd-MMM-yyyy") & "','" & Format(Dtp_ToVoucherDate.Value, "dd-MMM-yyyy") & "','T'"
                VCONN.dataOperation(6, strsql, "ACCOUNTPOSTING")
                'Me.Cursor = Cursors.Default

            Else
                Sqlstring = "DELETE FROM PURCHASE_Journal_Entry"
                Insert(0) = Sqlstring

                Sqlstring = "INSERT INTO PURCHASE_Journal_Entry(VOUCHERNO,VOUCHERDATE,VOUCHERCATEGORY,VOUCHERTYPE,ACCOUNTCODE,ACCOUNTCODEDESC,SLCODE,SLDESC,CREDITDEBIT,AMOUNT,DESCRIPTION,VOID,ADDUSERID,ADDDATETIME) SELECT GRNDETAILS,GRNDATE,'PJV','PJV','" & Trim(gCreditors) & "','SUNDRY CREDITORS',SUPPLIERCODE,SUPPLIERNAME,'CREDIT',BILLAMOUNT,'PURCHASE AUTO POSTING - INV NO: ' + SUPPLIERINVNO + ' DT: ' + CONVERT(VARCHAR(11),SUPPLIERDATE,6),VOID,'" & gUsername & "',GETDATE() FROM GRN_HEADER WHERE grndate between '" & Format(Dtp_FromVoucherDate.Value, "yyyy-MMM-dd") & "' and '" & Format(Dtp_ToVoucherDate.Value, "yyyy-MMM-dd") & "'"

                ReDim Preserve Insert(Insert.Length)
                Insert(Insert.Length - 1) = Sqlstring

                'Sqlstring = "INSERT INTO PURCHASE_Journal_Entry(VOUCHERNO,VOUCHERDATE,VOUCHERCATEGORY,VOUCHERTYPE,ACCOUNTCODE,ACCOUNTCODEDESC,SLCODE,SLDESC,CREDITDEBIT,AMOUNT,DESCRIPTION,VOID,ADDUSERID,ADDDATETIME) SELECT GRNDETAILS,GRNDATE,'PJV','PJV',GLACCOUNTCODE,GLACCOUNTNAME,SUPPLIERCODE,SUPPLIERNAME,'DEBIT',BILLAMOUNT,'PURCHASE AUTO POSTING - INV NO: ' + SUPPLIERINVNO + ' DT: ' + CONVERT(VARCHAR(11),SUPPLIERDATE,6),VOID,'" & gUsername & "',GETDATE() FROM GRN_HEADER WHERE MONTH(grndate)=" & Month(Dtp_ToVoucherDate.Value) & " and year(grndate)=" & Year(Dtp_ToVoucherDate.Value)

                Sqlstring = "INSERT INTO PURCHASE_Journal_Entry(VOUCHERNO,VOUCHERDATE,VOUCHERCATEGORY,VOUCHERTYPE,ACCOUNTCODE,ACCOUNTCODEDESC,SLCODE,SLDESC,CREDITDEBIT,AMOUNT,DESCRIPTION,VOID,ADDUSERID,ADDDATETIME) SELECT GRNDETAILS,GRNDATE,'PJV','PJV',GLACCOUNTCODE,GLACCOUNTNAME,'','','DEBIT',BILLAMOUNT,'PURCHASE AUTO POSTING - INV NO: ' + SUPPLIERINVNO + ' DT: ' + CONVERT(VARCHAR(11),SUPPLIERDATE,6),VOID,'" & gUsername & "',GETDATE() FROM GRN_HEADER WHERE grndate between '" & Format(Dtp_FromVoucherDate.Value, "yyyy-MMM-dd") & "' and '" & Format(Dtp_ToVoucherDate.Value, "yyyy-MMM-dd") & "'"
    ...
    Private Sub cmd_AccPosting_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmd_AccPosting.Click
        Try
            ...
            'Else
            Dim K As Integer
            SQLSTRING = "select distinct accountcode,ACCOUNTCODEDESC,SLCODE,SLDESC,ISNULL(COSTCENTERCODE,'') AS COSTCENTERCODE,ISNULL(COSTCENTERDESC,'') AS COSTCENTERDESC FROM purchase_journal_entry"
            SQLSTRING = SQLSTRING & " WHERE VOUCHERDATE between '" & Format(Dtp_FromVoucherDate.Value, "dd MMM yyyy") & "' AND '" & Format(Dtp_ToVoucherDate.Value, "dd MMM yyyy") & "' AND ISNULL(FREEZEUSERID,'')<>'Y' ORDER BY accountcode"
            VCONN.getDataSet(SQLSTRING, "ACCODE")
            ROWCOUNT = 1
            If gdataset.Tables("ACCODE").Rows.Count > 0 Then
    ...
    Private Sub CMD_POST_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMD_POST.Click
        Try
            ...
            'gcommand.Parameters.Add(New SqlParameter("@STARTDATE", SqlDbType.DateTime)).Value = Format(CDate(Dtp_FromVoucherDate.Value), "dd-MMM-yyyy")
            'gcommand.Parameters.Add(New SqlParameter("@ENDDATE", SqlDbType.DateTime)).Value = Format(CDate(Dtp_ToVoucherDate.Value), "dd-MMM-yyyy")
            'gcommand.Parameters.Add(New SqlParameter("@POSTFLAG", SqlDbType.VarChar)).Value = "P"
            gcommand.Parameters.Add(New SqlParameter("@sdt", SqlDbType.DateTime)).Value = Format(CDate(Dtp_FromVoucherDate.Value), "dd-MMM-yyyy")
            gcommand.Parameters.Add(New SqlParameter("@edt", SqlDbType.DateTime)).Value = Format(CDate(Dtp_ToVoucherDate.Value), "dd-MMM-yyyy")
            gcommand.Parameters.Add(New SqlParameter("@POSTFLAG", SqlDbType.VarChar)).Value = "P"
            gcommand.ExecuteNonQuery()
            VCONN.closeConnection()
            '--------------------------------------------------------------------------'
